...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for (int i = 0; i < height; i++) {
    ...
    DMA[3].cnt = width | DMA_ON | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT; //check height DMA calls
  }
}
...
void fillScreenDMA(volatile u16 color) {
  // TODO: IMPLEMENT
  //UNUSED(color);
  DMA[3].src = &color; //addr of color
  DMA[3].dst = videoBuffer; //full screen
  DMA[3].cnt = (WIDTH*HEIGHT)| DMA_ON | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT;
}
